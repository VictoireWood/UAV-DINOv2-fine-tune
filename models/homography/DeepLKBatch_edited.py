...
def warp_hmg(img, p):
	...
	# 这个函数比较重要
	ts_1 = time.time()
	img_warp, mask, xy_patch_org_cor = grid_bilinear_sampling(img, X_warp, Y_warp, h, w)
...
def warp_hmg_lite(map, p, img_h, img_w):
	# perform warping of img batch using homography transform with batch of parameters p
	# img [in, Tensor N x C x H x W] : batch of images to warp
	# p [in, Tensor N x 8 x 1] : batch of warp parameters
	# img_warp [out, Tensor N x C x H x W] : batch of warped images
	...
	batch_size, k, h, w = map.size()
	aspect = img_w / img_h
	multi = h / img_h
	margin = (w - h * aspect) / 2


	if isinstance(map, torch.autograd.Variable):
		if USE_CUDA:
			x = Variable(torch.arange(img_w).cuda())
			y = Variable(torch.arange(img_h).cuda())
		else:
			x = Variable(torch.arange(img_w))
			y = Variable(torch.arange(img_h))
	else:
		x = torch.arange(img_w)
		y = torch.arange(img_h)

	x = torch.round(x * multi + margin)
	# x = torch.round(x * multi)
	y = torch.round(y * multi)
	X, Y = meshgrid(x, y)
	...
	X_warp = X_warp.view(batch_size, img_h, img_w) + (w - 1) / 2
	Y_warp = Y_warp.view(batch_size, img_h, img_w) + (h - 1) / 2

	# the core function
	img_warp, mask, xy_patch_org_cor = grid_bilinear_sampling(map, X_warp, Y_warp, img_h, img_w)

	return img_warp, mask, xy_patch_org_cor


def warp_hmg_Noncentric(map, p, xy_cor_curr, img_w = 300, img_h = 225):
	# 对于warp_hmg的自定义重写，
...
def grid_bilinear_sampling(A, x, y, img_h, img_w):
	# 修改
	A = A.to(device)
	# k的大小是channel的大小
	...
	x_norm = x / ((w - 1) / 2) - 1
	y_norm = y / ((h - 1) / 2) - 1
	grid = torch.cat((x_norm.view(batch_size, img_h, img_w, 1), y_norm.view(batch_size, img_h, img_w, 1)), 3)
	...
	if k > 3:
		...
		x_corner_1 = round(x[0, 0, 0].item())
		y_corner_1 = round(y[0, 0, 0].item())
		x_corner_2 = round(x[0, 0, round(img_w-1)].item())
		y_corner_2 = round(y[0, 0, round(img_w-1)].item())
		x_corner_3 = round(x[0, round(img_h-1), round(img_w-1)].item())
		y_corner_3 = round(y[0, round(img_h-1), round(img_w-1)].item())
		x_corner_4 = round(x[0, round(img_h-1), 0].item())
		y_corner_4 = round(y[0, round(img_h-1), 0].item())
		x_patch_org_cor = round(x[0, round((img_h - 1) / 2), round((img_w - 1) / 2)].item())
		y_patch_org_cor = round(y[0, round((img_h - 1) / 2), round((img_w - 1) / 2)].item())
		# print(x_patch_org_cor, y_patch_org_cor)
	...
	corner_cor = [x_patch_org_cor, y_patch_org_cor]
	corner_1 = [x_corner_1, y_corner_1]
	corner_2 = [x_corner_2, y_corner_2]
	corner_3 = [x_corner_3, y_corner_3]
	corner_4 = [x_corner_4, y_corner_4]
	return Q.view(batch_size, k, img_h, img_w), in_view_mask, [corner_cor, corner_1, corner_2, corner_3, corner_4]
